...
class DemoDataService
{
     ...
     * Represents 2026-01-01 00:00:00 UTC.
     */
    private const BASE_EPOCH = 1767225600;
     ...
     */
    public function generateRecord(\DateTime $timestamp, int $id = 1): array
    {
        ...
        // Uptime: counts up from BASE_EPOCH (never negative)
        $uptimeSeconds = max(0, $ts - self::BASE_EPOCH);
     ...
     */
    public function generateHistory(int $limit = 100): array
    {
        ...
        for ($i = 0; $i < $limit; $i++) {
            ...
            $id  = self::ID_BASE - $i;
            $records[] = $this->generateRecord($dt, $id);
        }
     ...
     * @return array Array of formatted telemetry records
     */
    public function generateRange(string $from, string $to): array
    {
        ...
        for ($ts = $startTs; $ts <= $toTs; $ts += 30) {
            $dt = new \DateTime('@' . $ts);
            $dt->setTimezone(new \DateTimeZone('UTC'));
            $records[] = $this->generateRecord($dt, $idBase + $idx);
            $idx++;
